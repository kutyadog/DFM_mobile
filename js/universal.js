...
function loadNewSection() {
	...
	var n = xFeedList[activeSection].url.replace("http://","");
	xURL = 'denverfeed.php?x='+ n;
...
function DrawStoryList() {
	...
	xString +=	'</ul>';
	//xString +=	'</div>';
	xString +=	'	<footer><img class="dfm-logo" src="dfm_logo.png" />';
...
function viewStory(xnum) {
	if ( xInterface.allowUserEvent() ) {
		...
		*/
		...
		//console.dir("STORYLIST:" + StoryList[ activeStory ]);
		...
		xString += '<h4 class="flag dark">The Denver Post</h4>'
		...
		//<span class="fakelink" onclick="clickNewSection(1);">Home</span> / <span class="fakelink" onclick="clickNewSection(1);">News</span> / Story</div>';
		


		xString += '<div class="story_wrapper"><h1 class="story_headline">' + StoryList[ activeStory ].title + '</h1>';
		xString += '<p class="meta">';
		xString += '<span class="story_author">By Kirk Mitchell, The Denver Post</span>';
		xString += '<span class="story_pubdate">Published: ' + StoryList[ activeStory ].pubDate + '</span>';
		if(StoryList[ activeStory ] != '') {
			xString += '<span class="story_update">Updated: '+ StoryList[ activeStory ].lastUpdate[0] +'</span>';
		}
		xString += '</p><!-- .meta -->';
		if ( xURL ) xString += '<div class="main_image"><img src="'+ xURL +'" /></div><!-- .main-image -->';
		
		xString += '<div class="story_content">' + StoryList[ activeStory ].description + '</div><!-- #story_content -->';
...
function renderSectionsWindow() {
	...
	var xString = '<div class="list_header">Sections</div>';
